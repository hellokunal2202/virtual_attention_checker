...
def main():
    ...
    if "meetings" not in st.session_state or not st.session_state["meetings"]:
    ...
    meetings_df["start_time"] = pd.to_datetime(meetings_df["start_time"])

    # Meeting selection
    meeting_options = {
        idx: f"{row['title']} ({row['start_time'].strftime('%Y-%m-%d')})"
    ...
    if selected_idx is not None:
        ...
        participants = len(meeting["attendee_list"])
        ...
        ax.plot(
            df["timestamp"], df["engagement"], marker="o", color="#4CAF50", linewidth=2
        )
