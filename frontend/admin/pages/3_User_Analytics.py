...
def calculate_user_metrics(user_id, meeting_history=None):
    ...
    if meeting_history is None:
        ...
        meeting_history = [
            m for m in st.session_state["meetings"] if user_id in m["attendee_list"]
        ]
    ...
    if meetings_attended > 0:
        avg_engagement = round(
            sum(m.get("final_score", 75) for m in meeting_history) / meetings_attended,
            1,
        )
    else:
        avg_engagement = 0
...
def main():
    ...
    if "users_data" in st.session_state:
        user_list = st.session_state["users_data"]
    ...
    selected_user = st.selectbox(
        "Select User",
        options=user_list,
        format_func=lambda emp: f"{emp['username']}",
        key="user_selector",
    )
    ...
    if selected_user:
            ...
            meeting_history.append(
                {
                    ...
                    "attendee_list": [user_id],  # Add user to attendees
                }
            )
        ...
        meeting_options = {
            ...
            **{
                m[
                    "meeting_id"
                ]: f"{m['title']} ({m['start_time'].strftime('%Y-%m-%d %H:%M')})"
        ...
        if selected_meeting == "all":
            ...
            filtered_meetings = [
                m for m in meeting_history if m["meeting_id"] == int(selected_meeting)
            ]
            display_title = meeting_options[selected_meeting]

        # Calculate metrics using both old and new approaches
        meetings_attended, avg_score, trend, user_meetings = calculate_user_metrics(
            user_id, filtered_meetings
        )
        ...
        if selected_meeting == "all":
            ...
            chart_data = sorted(filtered_meetings, key=lambda x: x["start_time"])
            dates = [m["start_time"] for m in chart_data]
            ...
            with col3:
                engagement = (
                    "High"
                    if meeting["final_score"] >= 80
                    else "Medium" if meeting["final_score"] >= 65 else "Low"
                )
            ...
            time_labels = [
                f"{(meeting['start_time'].hour + (meeting['start_time'].minute + x*10)//60):02d}:{((meeting['start_time'].minute + x*10)%60):02d}"
            ...
            ax.axhline(
                y=meeting["final_score"], color="r", linestyle="--", label="Average"
            )
        ...
        st.subheader(
            "Meeting Details" if selected_meeting == "all" else "Meeting Summary"
        )
